...
class PacmanComponentState extends State<PacmanComponent>
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Container(
        constraints:
            BoxConstraints.tight(boxSize.calculateMaxSizeGame(sizeBoxOuter)),
        child: Stack(
  ...
  setupAnimationPlayer(
    ...
    animation =
        Tween<double>(begin: start, end: end).animate(animationController);
  ...
  updateDragDown(Offset offset, {bool start = false}) {
    if (start) {
      ...
      playerNotifier.value.position!
          .setDirectionInt((offset - offsetDragStart!));
  ...
  generateList() {
      ...
      return List.generate(
        ...
        (columnIndex) {
          ...
          if (!noRouteOuter) {
            ...
            try {
              boxValue = routerInner
                  .asMap()
                  .entries
                  .firstWhere((element) => element.key == columnIndex)
                  .value;
            } catch (ex) {
              boxValue = -1;
            }
            ...
            switch (boxValue) {
              ...
              default:
                if (boxValue >= 0) {
                  enemiesNotifier.value[boxValue - 4].position!
                      .setBoxPos(BoxPos(index, columnIndex), sizePerBox);
                }
            }
          } else {
            gotWall = true;
    ...
    playerNotifier.value.position!.setBoxPos(
      playerPos!,
      sizePerBox,
    );
    playerNotifier.value.position!.setOffsetDefault();

    playerNotifier.value.setBoxes(boxesNotifier.value
    ...
    barriers = boxesNotifier.value
        .map((e) => e
            .where((element) => element.isWall)
            .map((e) => Offset(e.position!.columnIndex.toDouble(),
                e.position!.rowIndex.toDouble()))
            .toList())
        .toList();
  }

  void setTimerProcess() {
    List<Box> boxess = boxesNotifier.value
        .expand((element) => element)
        .where((element) => !element.isWall)
        .toList();

    timerPlayer =
        Timer.periodic(const Duration(milliseconds: 50), (timerEnemies) {
      if (boxesNotifier.value.isNotEmpty && playerNotifier.value.flagMove()) {
        if (playerNotifier.value.position?.offset != Offset.zero) {
          if ((playerNotifier.value.powerUp && timerEnemies.tick % 1 != 0) ||
              (!playerNotifier.value.powerUp && timerEnemies.tick % 2 != 0)) {
            return;
          }
          ...
          boxesNotifier.value
              ...
              .where((element) => element.checkoffsetInRange(
                    playerNotifier.value.position!.offset!.translate(
                        sizePerBox.shortestSide / 2,
                        sizePerBox.shortestSide / 2),
                  ))
      ...
      if (boxesNotifier.value.isNotEmpty) {
        ...
        enemiesNotifier.value =
          ...
          if (e.position?.offset != Offset.zero && e.flagMove()) {
            bool hitPlayer = !e.playerPowerUp &&
                !e.die &&
                e.position!.positionInCenter(
            ...
            bool eatByPlayer = e.playerPowerUp &&
                e.position!.positionInCenter(
                    sizePerBox, playerNotifier.value.position!);
            ...
            if (!e.die &&
                ((e.playerPowerUp && timerEnemies.tick % 3 != 0) ||
                    (!e.playerPowerUp && timerEnemies.tick % 2 != 0))) {
              return e;
            }
